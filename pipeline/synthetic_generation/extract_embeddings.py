...
python pipeline\extract_embeddings.py `
  --audio "C:/Users/beiba\Desktop/baseline-polysim/Poly-sim/FOP/pipeline_output/00000/tts_audio_16k.wav" `
  --video "C:/Users/beiba/Desktop/baseline-polysim/Poly-sim/FOP/pipeline_output/00000/lipsynced.mp4" `
  --speaker_id id0001 `
  --language German `
  --video_id h_vamljclHE `
  --root_dir "C:/Users/beiba/Desktop/baseline-polysim/Poly-sim/FOP"
"""
...
def extract_facenet(
    ...
    """
    Extract FaceNet embeddings from every frame_step-th frame.

    Example:
        frame_step=25 means process frames 0, 25, 50, 75, ...
    """
    ...
    logger.info(
        "Extracted %d FaceNet embeddings from %s using every %d-th frame",
        len(results),
        video_path,
        frame_step,
    )
...
def save_ecapa_embedding(
    ...
    folder = "ecappafeats_synthetic" if synthetic else "ecappafeats"
...
def save_facenet_embeddings(
    ...
    folder = "facenetfeats_synthetic" if synthetic else "facenetfeats"
...
def save_embeddings(
    ...
    ecapa_path = save_ecapa_embedding(
        audio_emb,
        root_dir,
        speaker_id,
        language,
    ...
    face_embs = extract_facenet(video_path, frame_step=25)
    if len(face_embs) == 0:
        logger.warning(
        "No face found using every 25th frame. Retrying every 2nd frame: %s",
        video_path,
    )
        face_embs = extract_facenet(video_path, frame_step=2)

    if len(face_embs) == 0:
        raise RuntimeError(f"No face embeddings extracted from video: {video_path}")

    facenet_paths = save_facenet_embeddings(
        face_embs,
        root_dir,
        speaker_id,
        language,
