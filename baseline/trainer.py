...
class Trainer:
    ...
    def train_epoch(self, loader, alpha, logger=None, epoch=None):
        ...
        for audio, face, labels in pbar:
            audio = audio.to(self.config.device, non_blocking=True).float()
            face = face.to(self.config.device, non_blocking=True).float()
            labels = labels.to(self.config.device, non_blocking=True).float()
            ...
            labels = labels.long()
            #print("logits dtype:", logits.dtype, "labels dtype:", labels.dtype)
            loss = (
